...
function upload(buffer: Buffer, fileName = "bukti.jpg", token = employeeToken) {
  return request(app)
...
describe("POST /api/v1/leave-requests/:id/attachments", () => {
  ...
  it("mengunggah gambar PNG yang sah", async () => {
    const res = await upload(PNG, "bukti.png");

    expect(res.status).toBe(201);
  });

  it("menolak berkas yang bukan gambar meski ekstensinya jpg", async () => {
    const res = await upload(Buffer.from("%PDF-1.7 palsu"), "bukti.jpg");
  ...
  it("menolak skrip yang menyamar sebagai gambar", async () => {
    const res = await upload(Buffer.from("<?php echo 1; ?>"), "bukti.png");
  ...
  it("mengizinkan admin", async () => {
    ...
    const res = await upload(JPEG, "bukti.jpg", adminToken);
...
describe("GET /api/v1/leave-attachments/:id/url", () => {
  function requestSignedUrl(token = employeeToken) {
    return request(app)
  ...
  it("mengizinkan admin melihat lampiran siapa pun", async () => {
    ...
    const res = await requestSignedUrl(adminToken);

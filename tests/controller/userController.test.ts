...
describe("POST /api/v1/auth/login", () => {
    ...
    it("menolak email yang tidak terdaftar", async () => {
        (userModel.findByEmail as jest.Mock).mockResolvedValue(null as never);
        const res = await request(app).post("/api/v1/auth/login").send({ email: "tidakada@awan.io", password: "password123" });
    ...
    it("menolak password yang salah", async () => {
        ...
        const res = await request(app).post("/api/v1/auth/login").send({ email: "ismail@awan.io", password: "passwordsalah" });
        expect(res.status).toBe(401);
    });
    ...
    it("memberi pesan yang sama untuk email tidak terdaftar dan password salah", async () => {
        const hashed = await hashPassword("password123");
        (userModel.findByEmail as jest.Mock).mockResolvedValue(null as never);
        const resEmail = await request(app).post("/api/v1/auth/login").send({ email: "tidakada@awan.io", password: "password123" });
        (userModel.findByEmail as jest.Mock).mockResolvedValue({
            ...fakeUser,
            password: hashed,
        } as never);
        const resPassword = await request(app).post("/api/v1/auth/login").send({ email: "ismail@awan.io", password: "salah" });
    ...
    it("menolak akun yang belum disetujui", async () => {
        ...
        const res = await request(app).post("/api/v1/auth/login").send({ email: "ismail@awan.io", password: "password123" });
        expect(res.status).toBe(401);
    ...
    it("mencatat waktu login terakhir", async () => {
        ...
        await request(app).post("/api/v1/auth/login").send({ email: "ismail@awan.io", password: "password123" });
